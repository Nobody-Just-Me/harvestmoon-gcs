...
public sealed class DesktopVoiceRecognitionService : IVoiceRecognitionService, IDisposable
{
    ...
    private static string ResolveRecognizerScriptPath()
    {
        ...
        var candidates = new[]
        {
            ...
            Path.Combine(AppContext.BaseDirectory, "HarvestmoonGCS", "pia_stt_listener.py")
        };
        ...
        foreach (var root in roots)
        {
            ...
            for (var depth = 0; depth < 8; depth++)
            {
                ...
                var nested = SafeFullPath(Path.Combine(current, "HarvestmoonGCS", "pia_stt_listener.py"));
