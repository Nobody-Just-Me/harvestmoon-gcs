...
public class AndroidCameraService : ICameraService
{
    ...
    private const int DefaultFrameWidth = 1280;
    private const int DefaultFrameHeight = 720;
    ...
    private void ConfigureImageReader(string cameraId)
    {
        ...
        _imageReader.SetOnImageAvailableListener(new ImageAvailableListener(this), _mainHandler);
    }
    ...
    private async Task<CameraDevice?> OpenCameraInternalAsync(string cameraId)
    {
        ...
        try
        {
            _cameraManager.OpenCamera(cameraId, new CameraStateCallback(this, tcs), _mainHandler);
        }
        catch (Exception ex)
        {
    ...
    private async Task<bool> StartPreviewSessionAsync()
    {
        ...
        try
        {
            ...
            _cameraDevice.CreateCaptureSession(outputs, new SessionStateCallback(this, tcs), _mainHandler);
        }
        catch (Exception ex)
        {
    ...
    private sealed class SessionStateCallback : CameraCaptureSession.StateCallback
    {
        ...
        public override void OnConfigured(CameraCaptureSession session)
        {
            try
            {
                ...
                session.SetRepeatingRequest(_service._previewRequest, null, _service._mainHandler);
