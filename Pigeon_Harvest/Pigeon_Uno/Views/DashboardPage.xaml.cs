...
public sealed partial class DashboardPage : Page
{
    ...
    private void InitializeDashboardMap()
    {
        ...
        // Default tile provider matches MapPage default center (Surabaya) fallback to FlightViewModel home
        DashboardMapControl.SetTileProvider(SkiaMapControl.MapTileProvider.ArcGISTopographic);
        ...
        var initialLat = IsValidCoordinate(_centerLat, _centerLon) ? _centerLat : -7.2754;
        var initialLon = IsValidCoordinate(_centerLat, _centerLon) ? _centerLon : 112.7947;
    ...
    private void SyncWaypointsFromViewModel()
    {
        ...
        {
            // Show 3 demo waypoints so the panel isn't empty before any real mission is loaded.
            DashboardMapControl.AddWaypointMarker(1, _centerLat + 0.0012, _centerLon + 0.0009, 150, "WP");
            DashboardMapControl.AddWaypointMarker(2, _centerLat + 0.0019, _centerLon + 0.0017, 170, "WP");
            DashboardMapControl.AddWaypointMarker(3, _centerLat + 0.0010, _centerLon + 0.0024, 165, "WP");
            return;
        }
    ...
    private void SyncGeofenceFromViewModel()
    {
        ...
        {
            // Default demo circle around current center if user hasn't drawn one yet.
            DashboardMapControl.SetGeofence(true, _centerLat, _centerLon, 180);
            return;
        }

        if (_mapViewModel.GeofenceType == Pigeon_Uno.Core.Models.GeofenceType.Circular)
        {
    ...
    private const int VideoFrameIntervalMs = 33; // ~30 FPS
    ...
    private void OnDashboardFrameReceived(object? sender, byte[] frameData)
    {
        ...
        if (_aiOn && _harvestFunctionalService?.IsYoloOptionEnabled == true && DateTime.Now - _lastAnalysisTime > TimeSpan.FromMilliseconds(500) && !_analysisRunning)
        {
        ...
        try
        {
            ...
            DispatcherQueue.TryEnqueue(() =>
            {
                ...
                FpsHudText.Text = _aiOn
                    ? (_harvestFunctionalService!.IsYoloRuntimeReady ? $"YOLOv8 · {boxes.Count} det" : "YOLO fallback")
                    : "YOLO paused";
    ...
    {
        SummaryPestCount.Text = "12";
        SummaryStressCount.Text = "8";
        SummaryIrrigationCount.Text = "3";
        SummaryHealthyCount.Text = "47";
        SummaryPriorityText.Text = "High";
        SummaryConfidenceText.Text = "92%";

        NdviHealthyBar.Value = 62;
        NdviHealthyText.Text = "62%";
        NdviModerateBar.Value = 24;
        NdviModerateText.Text = "24%";
        NdviStressedBar.Value = 9;
        NdviStressedText.Text = "9%";
        NdviCriticalBar.Value = 5;
        NdviCriticalText.Text = "5%";
        NdviAvgText.Text = "0.68";
        NdviCoverageText.Text = "3.4 ha";
    }
