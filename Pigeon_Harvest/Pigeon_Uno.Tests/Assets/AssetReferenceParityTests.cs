...
namespace Pigeon_Uno.Tests.Assets;
...
public class AssetReferenceParityTests
{
    ...
    private static string ResolveAppProjectRoot()
    {
        ...
        while (current != null)
        {
            var candidate = Path.Combine(current.FullName, "Pigeon_Uno", "Pigeon_Uno.csproj");
            if (File.Exists(candidate))
            {
                return Path.Combine(current.FullName, "Pigeon_Uno");
            }

            current = current.Parent;
        }

        throw new DirectoryNotFoundException("Unable to locate Pigeon_Uno app project root.");
    }
